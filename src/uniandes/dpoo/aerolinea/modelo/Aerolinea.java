...
public class Aerolinea
{
    ...
    public Vuelo getVuelo(String codigoRuta, String fechaVuelo) {
        for (Vuelo vuelo : vuelos) {
            if (vuelo.getRuta().getCodigoRuta().equals(codigoRuta) && vuelo.getFecha().equals(fechaVuelo)) {
                return vuelo; // Retorna el vuelo si ambos parámetros coinciden
            }
        }
        return null; // Si no se encuentra ningún vuelo, retorna null
    }
    ...
    {
    	List<Tiquete> tiquetesRecoleccion = new ArrayList<>(); // Lista donde se recolectarán todos los tiquetes

        // Recorrer todos los vuelos
        for (Vuelo vuelo : vuelos) {
            // Obtener el mapa de tiquetes del vuelo actual
            Map<String, Tiquete> tiquetesVuelo = vuelo.getTiquetes();

            // Añadir todos los tiquetes de este vuelo a la lista de recolección
            tiquetesRecoleccion.addAll(tiquetesVuelo.values());
        }

        // Retornar todos los tiquetes recolectados
        return tiquetesRecoleccion;
    ...
    {
    	CentralPersistencia persistencia = CentralPersistencia.getPersistenciaAerolinea(tipoArchivo); // Obtener la implementación de persistencia según el tipo
    ...
    {
    	IPersistenciaAerolinea persistencia = CentralPersistencia.getPersistenciaAerolinea(tipoArchivo); // Obtener la implementación de persistencia según el tipo
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        if (ruta == null) {
            throw new Exception("Ruta no encontrada: " + codigoRuta);
        }
        ...
        if (avion == null) {
            throw new Exception("Avión no encontrado: " + nombreAvion);
        }

        // Verificar que el avión no esté ocupado en esa fecha
        for (Vuelo vuelo : vuelos) {
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (cliente == null) {
            throw new Exception("Cliente no encontrado: " + identificadorCliente);
        }

        // Buscar el vuelo
        Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo == null) {
            throw new Exception("Vuelo no encontrado para la ruta " + codigoRuta + " en la fecha " + fecha);
        }

        // Verificar la capacidad del vuelo
        if (vuelo.hayEspacioDisponible()) {
            throw new VueloSobrevendidoException("No hay suficientes asientos disponibles");
        }

        // Calcular el precio según la temporada
        int precioTiquete = vuelo.calcularPrecioPorTemporada(fecha);
        int total = precioTiquete * cantidad;

        // Asignar los tiquetes al cliente y vuelo
        vuelo.venderTiquetes(cliente, cantidad);
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
    	Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo != null) {
            vuelo.marcarComoRealizado(); // Método que marca el vuelo como realizado
        } else {
            System.out.println("No se encontró el vuelo para la ruta " + codigoRuta + " en la fecha " + fecha);
        }
    }
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        for (Tiquete tiquete : cliente.getTiquetes()) {
            if (!tiquete.estaUtilizado()) { // Verificar si el tiquete aún no se ha usado
                saldoPendiente += tiquete.getValor();
            }
        }
	...
	public void setRutas(HashMap<String, Ruta> rutas) {
		this.rutas = rutas;
	}
